  ...
  constructor() {
   ...
   getProductsByCategoryId(cid:number):IProduct[]{
    if(cid===0){
      ...
      return this.products.filter(p=>Number (p.categoryId)===Number(cid) );

    }

   }
   deleteProduct(pid:number):void{
    let selectedProduct=this.products.find(p=>p.id===pid);
    let index=this.products.indexOf(selectedProduct as IProduct)
    this.products.splice(index,1);
    this.getAllProducts(); // to redirect to all prods
